...
function ReadChunk(bc) { // bc: Buffer
	...
	while (true) { // Read protos
		var protolen = ReadULEB(Chunk) // Proto lenght
	...
	for (var Proto of chunk.protos) { // Iterating protos
		for (var inst of Proto.insts) { // Each instructions
			...
			if (!(opCode in opcodesWeight)) {
				...
				LineHeatMap[linenum] += weight // Sum
				MaxWeight = Math.max(MaxWeight, LineHeatMap[linenum])
			}
		}
	}
...
const fs = require("fs", noop = () => { });
console.log(MakeHeatMap(ReadChunk(fs.readFileSync("C:/Users/Spar/Desktop/normalwow.lua.bytes")))[0])